...
public class IntroScene : MonoBehaviour {
	...
	void OnGUI(){
		if (startTime < dTime) {
			...
			if(startTime>dTime-1) intens = dTime-startTime;
			GUI.color = new Color(intens,intens,intens,1);
			GUI.skin.label.alignment = TextAnchor.MiddleCenter;

			GUI.skin.label.fontSize =80;
			GUI.skin.label.normal.textColor = Color.white;
			GUI.Label(new Rect(50,50,Screen.width-100,100),"Maze Of God");
			GUI.skin.label.fontSize =60;
			GUI.Label(new Rect(50,120,Screen.width-100,100),"Deus의 미로 전략 액션 RPG");
			GUI.skin.label.fontSize=40;
			GUI.Label(new Rect(50,170,Screen.width-100,Screen.height-200),"한성대학교 컴퓨터공학과");
		} else {
			float intens =1;
			if (startTime<dTime+1)	intens = startTime-dTime;
			if (startTime>splashTime+dTime-1)	intens = dTime+splashTime-startTime;
			GUI.color = new Color(intens,intens,intens,1);
	...
	void OnGUI(){
		if (startTime<disclaimerTime){
			...
			GUI.skin.label.normal.textColor = Color.white;
			GUI.Label(new Rect(50,50,Screen.width-100,100), "하이 GAME CONTAINS MATERIAL WHICH IS COPYRIGHT NINTENDO .PLC AND GAMEFREAK .INC");
